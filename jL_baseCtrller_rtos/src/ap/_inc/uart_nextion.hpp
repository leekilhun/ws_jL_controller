...
namespace NXLCD
{
	...
	| :--- |:-----|:-----|:---- | :---------- |:-------------|:-------------| :--  |
	| 0x48 |1 byte|1 byte|1 byte| 2 byte      |Data 0～Data n|2 byte(crc 16)| 0x4C |
	...
	constexpr uint8_t CMD_STX = 0x48;
...
#define AP_UART_NEXTION_LOCK_END
	...
	public:
		...
		struct cfg_t
		{
			...
			std::array <uint8_t,CMD_MAX_PACKET_LENGTH> *ptr_databuff;
		};
	...
	public:
		...
		inline bool SendCmd(uint8_t cmd, uint8_t *p_data, uint32_t length)
		{
			uint32_t value = 0;
		...
		inline bool SendCmdRxResp(uint8_t cmd, uint8_t *p_data, uint32_t length, uint32_t timeout = 100){
		...
		inline bool SendDataRxResp(uint8_t *p_data, uint8_t length, uint32_t timeout = 100){
			m_packet_sending_ms = millis();
			if(uartWrite(m_cfg.ch, p_data, length))
			{
				uint32_t pre_ms = millis();
				bool result = true;
				while (receivePacket() == false)
		...
		inline bool SendData(uint8_t *p_data, uint8_t length){
			m_packet_sending_ms = millis();
			if(uartWrite(m_cfg.ch, p_data, length))
			{
				return true;
		...
		inline bool receivePacket()	{
			...
			*m_cfg.ptr_databuff = data_arry;
			...
			if (millis() - m_packet.prev_ms >= 100)
			{
				...
				rx_packet->buffer.fill(0);//memset(&rx_packet->buffer[0], 0x00, CMD_MAX_PACKET_LENGTH);//
			}
				...
				switch (m_packet.state)
				{
					case STATE_WAIT_STX:
					{
						...
						if (rx_data == CMD_STX)
						{
							...
							m_packet.data_cnt = 0;
							rx_packet->check_sum = 0xffff;
							rx_packet->buffer.fill(0);//memset(&rx_packet->buffer[0], 0, CMD_MAX_PACKET_LENGTH);
					...
					{
						if (rx_data < (uint8_t)uart_nextion::page_e::page_max)
						{
							rx_packet->page_no = (page_e)rx_data;
