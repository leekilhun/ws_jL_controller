...
#define MOTOR1_STEP_PIN  GPIO_PIN_9
...
#define MOTOR2_GEAR_RATIO  15

#define ACCELERATION_RATE   100  // Steps/s^2
...
void TIM3_IRQHandler(void) {
	...
	if (motor1.steps_remaining > 0) {
		...
		if (motor1.direction == FORWARD) {
			HAL_GPIO_WritePin(GPIOC, MOTOR1_STEP_PIN, GPIO_PIN_SET);
			__NOP();
			__NOP();
			__NOP();
			__NOP();
			__NOP();
			HAL_GPIO_WritePin(GPIOC, MOTOR1_STEP_PIN, GPIO_PIN_RESET);
		} else {
			HAL_GPIO_WritePin(GPIOC, MOTOR1_DIR_PIN, GPIO_PIN_SET);
			__NOP();
			__NOP();
			__NOP();
			__NOP();
			__NOP();
			HAL_GPIO_WritePin(GPIOC, MOTOR1_DIR_PIN, GPIO_PIN_RESET);
		}
	}
	...
	if (motor2.steps_remaining > 0) {
		...
		if (motor2.direction == FORWARD) {
			...
			HAL_GPIO_WritePin(GPIOC, MOTOR2_DIR_PIN, GPIO_PIN_SET);
			__NOP();
			__NOP();
			__NOP();
			__NOP();
			__NOP();
			HAL_GPIO_WritePin(GPIOC, MOTOR2_DIR_PIN, GPIO_PIN_RESET);
		}
	}
}
...
int main(void) {
	...
	move(&motor1, FORWARD, 2000*10);
	set_speed(&motor1, MAX_SPEED);
	move(&motor2, FORWARD, 2000*10);
