...
void cliTimer(cli_args_t *args)
{
  ...
  if(args->argc == 6)
  {
  	...
  	regist_val = (uint8_t)args->getData(2)<<24;
  	regist_val |= (uint8_t)args->getData(3)<<16;
  	regist_val |= (uint8_t)args->getData(4)<<8;
  	regist_val |= (uint8_t)args->getData(5)<<0;
  ...
  if (ret == false)
  {
    ...
    cliPrintf( "timer set_PSC 0~%d byte3 byte2 byte1 byte0 \n", TIM_MAX_CH-1);
    cliPrintf( "timer set_ARR 0~%d byte3 byte2 byte1 byte0 \n", TIM_MAX_CH-1);
